...
/*        if(jobNameFull.split("_").length != 6)
        ...
        for(int i = 0 ; i< timeout/20; i++){
            ...
            System.out.println("Status of the job "+rs.getStatusMessage());
            	System.out.println("Executing modified code");

            if(rs.getStatusMessage().equalsIgnoreCase("In Progress"))
                {
                     continue;
     
                 }
                         else if(rs.getStatusMessage().equalsIgnoreCase("Success"))
